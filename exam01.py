...
# 2) Django : 모든 기능이 포함(flask보다 10~20 배 무겁다)

# 가상 환경 변경
# 우측 하단 3.9.13 혹은 가상환경 이름을 클릭
#Ctrl + Shift + P -> 인터프리터 검색 -> 하만
...
def login():
    if request.method == "GET":
        ...
        # 페이지 이동 : redirect
        # 페이지가 이동하더라도 정보를 남겨 사용


        login_id = request.args["login_id"]
        login_pw = request.args["login_pw"]
        if login_id == "mintaewon" and login_pw == "0519":
            #로그인 성공
